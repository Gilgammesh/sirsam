...
public class SesionDao {
    ...
    public List<Sesion> getLoginActivos(String query) {
        SqlSession session = ssfWeb.openSession();
        List<Sesion> list = (List<Sesion>) new ArrayList<Sesion>();
        HashMap hm = new HashMap();
        hm.put("c", query);
        try {
            list = session.selectList("Sesion.getLoginActivos", hm);
        } finally {
            session.close();
        }
        return list;
    ...
    public List<Sesion> getNavegaActivos(String query) {
        SqlSession session = ssfWeb.openSession();
        List<Sesion> list = (List<Sesion>) new ArrayList<Sesion>();
        HashMap hm = new HashMap();
        hm.put("c", query);
        try {
            list = session.selectList("Sesion.getNavegaActivos", hm);
        } finally {
            session.close();
        }
        return list;
    ...
    public void update(Sesion bean) {
        try (SqlSession session = ssfWeb.openSession()) {
            session.update("Sesion.update", bean);
            session.commit();
        }
    }
