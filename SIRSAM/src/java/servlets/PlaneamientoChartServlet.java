 ...
 */
@WebServlet(name = "PlaneamientoChartServlet", urlPatterns = {"/PlaneamientoChart"})
public class PlaneamientoChartServlet extends HttpServlet {
    ...
    public void barV2D(HttpServletRequest request, HttpServletResponse response,
        ...
        try (OutputStream out = response.getOutputStream()) {
            ...
            PlaneamientoGraficas graph = new PlaneamientoGraficasDao().getGraficaBarv2d(query);

            String queryG = graph.getGrafQuery().replace("{indi}", indi_id);
            List<PlaneamientoMetaData> list = new PlaneamientoMetaDataDao().getListaDatos(queryG);

            String queryF = " WHERE a.indi_id = " + indi_id + " AND a.inva_formula = TRUE AND a.inva_estado = 'A' ";
            Planeamiento varF = new PlaneamientoDao().getIndicadorVariables(queryF);
    ...
    public void barV2DFicha(HttpServletRequest request, HttpServletResponse response,
        ...
        try (OutputStream out = response.getOutputStream()) {
            ...
            Planeamiento indi = new PlaneamientoDao().getIndicador(queryM);
